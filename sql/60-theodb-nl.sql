...
--
-- M19 (ROADMAP-v2): `ai.nl_to_sql` (the LAST plpython3u, the anti-injection L1/L2/L4 core) is now implemented
-- in Rust by the `theodb_rs` extension (theodb_rs/src/nl.rs) — NOT here. `ai.nl_query` (L3 read-only sandbox)
-- STAYS a thin plpgsql keeper below (ADR-F — transaction-control + dynamic EXECUTE are SQL operations) and
-- calls the Rust `ai.nl_to_sql` at the SQL level. With this, the `theodb` surface is plpython3u-free and
-- `plpython3u` leaves `theodb.control` requires. L4 still delegates relation enumeration to PostgreSQL's
