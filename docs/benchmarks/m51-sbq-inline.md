...
> **Nota de comparação NÃO-CASADA (correção de review, HIGH-1/HIGH-2):** o SBQ foi varrido até `walk_ef = ef_search(400) × over_fetch(16) = 6400` candidatos no ground; f32/pgvector foram varridos **apenas até ef=400**. Os tetos de recall do f32/pgvector com um pool comparável (ef≈1600–6400) **NÃO estão nesta tabela** (UNBENCHMARKED — o `ef_search` do theodb/pgvector é capado em 1000, então o pool de 6400 do SBQ só é alcançável via `over_fetch`). Portanto: **NÃO leia o 0.9993 do SBQ como um "teto de recall superior" ao f32.** `over_fetch` (SBQ) e `ef_search` (f32) são o **mesmo lever** (largura do pool do ground); no limite, o f32 exato *é* o rerank de todo mundo. O que o 0.9993 prova é apenas que o **read path recupera recall corretamente** (o gate D3), não que o SBQ ranqueia melhor.

## 2. Leitura

- **Recall gate ≥0.99: ATINGIDO.** O SBQ-inline (Hamming no walk + rerank f32 exato) chega a **recall@10 = 0.9993** (of=16) / 0.9953 (of=8). Isso prova que o **read path recupera recall corretamente** — a navegação aproximada por Hamming + rerank exato não perde o NN quando o pool (`over_fetch`) é largo o suficiente, exatamente a predição do M40 (carrier-limited: carrier adequado → rerank recupera ~1.0). **Não é uma vantagem de recall sobre o f32** (ver a nota de comparação não-casada acima): o f32/pgvector num orçamento de pool comparável atingiriam recall comparável — não medido. *(O config 2-bit/ef=100 topa em ~0.52 — honest-negative: a navegação Hamming precisa de bits + carrier adequados; ver §3.)*
- **QPS a 25k: SBQ NÃO é mais rápido.** No recall casado ~0.95: SBQ (of=2) **0.946 @ 93.1 qps** ≈ f32 (ef=400) **0.932 @ 95.5 qps** — **paridade**, ambos atrás do pgvector (142). No gate ≥0.99, o SBQ custa QPS (38→27) porque o over_fetch alarga o pool e o rerank re-lê `walk_ef` páginas de element tuple f32 (dois passes por sobrevivente: Hamming no walk + f32 no rerank). **Isso é esperado e honesto**: a 25k o corpus f32 cabe em RAM (~12,8 MB) → a compressão 4× do SBQ 8-bit **não tem pressão de memória onde ganhar QPS**; ela só paga o custo do walk aproximado + rerank. **Exatamente o veredito do M50** (§4): "o ganho de QPS do SBQ só materializa sob pressão de memória".
...
- **Correção: PROVADA.** O read path (Hamming walk + rerank f32) é correto e **recupera recall ≥0.99** (0.9993 — não uma vantagem sobre o f32, ver §1 nota de comparação não-casada; é a prova de que o rerank recupera o recall que a navegação aproximada poderia perder), o gate central do M51. 12+ pg_test cobrem o formato v2, o build, o fold-preserve, o reloption, o fail-fast de corrupção e o recall do read path. Default `sbq_bits=0` (f32) → **zero regressão** em índices existentes.
