...
|---|---|---|
| The reconciler provisions a StatefulSet + Service, owner-referenced | `TestReconcile_CreatesStatefulSetAndService` (real envtest) | PASS |
...
| A cluster with no image fails fast (typed error + Phase=Error, no StatefulSet) | `TestReconcile_MissingImageFailsFast` | PASS |
| Status reflects readiness honestly under envtest (no kubelet → Phase=Initializing, EC-4) | `TestReconcile_StatusInitializingWithoutKubelet` | PASS |
| Pure builders produce the correct StatefulSet/Service (replicas/image/PVC/port/owner) | `TestBuildStatefulSet_*`, `TestBuildService_SelectorPort` | PASS |
...
=== RUN   TestReconcile_CreatesStatefulSetAndService   --- PASS (2.02s)   # real kube-apiserver+etcd
=== RUN   TestReconcile_Idempotent                     --- PASS (0.02s)
=== RUN   TestReconcile_ScaleUpUpdatesReplicas         --- PASS (0.02s)
=== RUN   TestReconcile_MissingImageFailsFast          --- PASS (0.01s)
=== RUN   TestReconcile_StatusInitializingWithoutKubelet --- PASS (0.01s)
ok  github.com/usetheodev/theo-db/operator/internal/controller  6.394s  coverage: 72.4% of statements
ok  github.com/usetheodev/theo-db/operator/cmd/theodbctl         coverage: 9.1% of statements
make test exit=0
```

> The `internal/controller` package — where all reconcile logic lives — is covered at **72.4%**.
...
> behavior-bearing code is the controller package.
...
# Full kustomize bundle (16 resources) validates server-side (after the namespace exists)
kustomize build operator/config/default | kubectl apply --dry-run=server -f -
#  → 16 resources validated, no errors
