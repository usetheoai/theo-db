...
- **Lakehouse Parquet (own-code)** — ler/escrever/agregar arquivos Parquet externos **own-code** via DataFusion/Arrow (`theodb.read_parquet`, `theodb.write_parquet`, `theodb.htap_refresh`/`olap`), **sem DuckDB** (M143 removeu o `pg_duckdb` por completo — ADR-0057). Uma imagem só, sem componente C++/httpfs; o leitor own-code custa +9 MB no binário vs os 118 MB do bundle DuckDB (`docs/benchmarks/parquet-reader-owncode-spike.md`).
