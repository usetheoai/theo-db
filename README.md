...
- **100% compatível com PostgreSQL no protocolo.** Seus drivers e aplicações falam com o TheoDB como falam com um PostgreSQL 18 — é o que ele é. **Medido com PgBouncer nos três modos** ([b055](wiki/benchmarks/b055-pgbouncer-compat.md)): sob `session` nada muda. Sob `transaction` e `statement`, o `SET theodb_hnsw.ef_search` de um cliente **passa a valer para o próximo cliente** que pegar aquela conexão — use `SET LOCAL` dentro da transação que faz a consulta. Isso é comportamento documentado do PgBouncer e vale para qualquer PostgreSQL; o que é nosso é a consequência, porque aqui o parâmetro que vaza decide **recall**. Os contadores de `theodb.explain_scan` foram medidos e **não** contaminam.
