/* theodb_rs 1.1.0 -> 1.2.0 — FULL-SCHEMA self-healing upgrade (M144 T1.1).
 *
 * Convenção do projeto (M137 + o oráculo CONV de scripts/test-upgrade.sh): os scripts de upgrade re-emitem o
 * schema COMPLETO de forma idempotente (CREATE OR REPLACE / IF NOT EXISTS / guards), de modo que um catálogo
 * incompleto CONVERGE para o estado completo (self-healing). Um delta-only (só os objetos novos) NÃO restaura
 * objetos dropados e falha o oráculo CONV — por isso este script re-emite o schema 1.1.0 inteiro (idêntico ao
 * corpo do 1.0.0->1.1.0, já provado) e ACRESCENTA a superfície lakehouse M143 (olap/read_parquet/write_parquet)
...
--
-- Convergente porque `1.0.0` rotula catálogos diferentes (a superfície foi 18→48→60→78 funções
-- com a versão congelada). Este script leva QUALQUER um deles ao estado 1.1.0, e é idempotente:
...
/* ===== M144: superfície lakehouse M143 (1.2.0 delta sobre o schema 1.1.0 acima) ===== */

-- src/parquet.rs:75  — theodb_rs::parquet::olap
...
-- src/parquet.rs:121 — theodb_rs::parquet::read_parquet
...
-- src/parquet.rs:168 — theodb_rs::parquet::write_parquet
...
-- src/parquet.rs:321 — least-privilege: the lakehouse primitives read/write server-side files, so they are
-- superuser-only (mirrors the extension_sql! REVOKE block that ships on a fresh install).
...
-- M144 T1.2 (review HIGH): `symqg_spike_bench` (a spike/benchmark fn that reads an arbitrary server-side file
-- via std::fs::read) shipped PUBLIC-executable in 1.1.0. Existing 1.1.0 installs get the least-privilege REVOKE
-- here on upgrade; fresh 1.2.0 installs get it via the extension_sql! block in bench_symqg.rs. Idempotent.
