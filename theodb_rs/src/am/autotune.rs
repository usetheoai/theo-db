...
/// Reset the backend-local pages-read accumulator (before a measured scan window).
fn reset_scan_pages() {
...
fn record_scan_stat(relid: i64, pages_read: i64, latency_us: i64, ef: i32) {
    Spi::run_with_args(
        "INSERT INTO theodb._index_scan_stats (relid, n_scans, sum_pages_read, sum_latency_us, last_ef) \
         VALUES ($1, 1, $2, $3, $4) \
        ...
        &[relid.into(), pages_read.into(), latency_us.into(), ef.into()],
    )
...
pub(crate) fn scan_stats(relid: i64, tbl: &str, col: &str, qvec: &str, ef: i32, k: i32) -> (i64, i64, i64) {
    if ef <= 0 || k <= 0 {
    ...
    let pages_read = read_scan_pages();
    record_scan_stat(relid, pages_read, latency_us, ef); // persist the observation (the collector)
    (pages_read, latency_us, results)
}
...
mod tests {
    ...
    fn scan_stats_records_real_pages_read() {
        ...
        // Measure one scan → real pages_read > 0 (the HNSW traverse fed the backend-local counter).
        let (pages, _latency, results) = scan_stats(0, "sst1", "e", &probes[0], 40, 5);
        ...
        let n: i64 = Spi::get_one("SELECT n_scans FROM theodb._index_scan_stats WHERE relid = 0").unwrap().unwrap_or(0);
