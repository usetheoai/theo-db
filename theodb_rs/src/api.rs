//! TheoDB extension SQL API surface (M25 — split from lib.rs): the `#[pg_extern]` entrypoints in
//! schema `theodb_rs` + the `extension_sql!` DDL that creates the public `theodb.*` / `ai.*` wrappers.
//! Moved VERBATIM from lib.rs — pgrx collects `#[pg_extern]`/`extension_sql!` from any module, so the
