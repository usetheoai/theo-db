...
const STOPWORDS: &[&str] = &[
    ...
    "are", "was", "were", "be", "been", "it", "this", "that", "these", "those", "we", "you", "they", "he", "she",
    "i", "if", "then", "so", "not", "no", "yes", "our", "their", "his", "her", "its", "when", "while", "after",
    "before",
];
...
fn extract_heuristic(text: &str, max_entities: usize, window: usize) -> (Vec<Entity>, Vec<Edge>) {
    ...
    let edges: Vec<Edge> = edge_order.iter().map(|k| (k.0.clone(), k.1.clone(), ew[k], String::new())).collect();
    (entities, edges)
}
...
#[pgrx::pg_schema]
mod theodb_rs {
    use super::{extract_dispatch, Entity};
    use pgrx::prelude::*;
    use std::collections::HashMap;
    ...
    fn _extract_entities(
        ...
        use_llm: bool,
        model: Option<&str>,
    ) -> TableIterator<'static, (name!(name, String), name!(normalized_name, String), name!(entity_type, String), name!(mention_count, i32))> {
    ...
    fn _extract_graph(
        ...
        use_llm: bool,
        model: Option<&str>,
    ) -> TableIterator<'static, (name!(src_normalized, String), name!(dst_normalized, String), name!(weight, i32), name!(description, String))> {
    ...
    #[pg_extern]
    fn _graph_upsert(workspace_id: &str, collection_id: &str, source_chunk_id: &str, text: &str, use_llm: bool) -> i64 {
...
mod tests {
    ...
    fn m110_extract_empty() {
        ...
        let n2: i64 = Spi::get_one("SELECT count(*) FROM ai.extract_entities('')").unwrap().unwrap();
    ...
    fn m110_upsert_idempotent() {
        ...
        let e1: i64 = Spi::get_one(&format!("SELECT theodb.graph_upsert('ws1','c1','chunk1',{})", quote(doc))).unwrap().unwrap();
        let nodes1: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_nodes WHERE workspace_id='ws1'").unwrap().unwrap();
        let edges1: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_edges WHERE workspace_id='ws1'").unwrap().unwrap();
        ...
        Spi::get_one::<i64>(&format!("SELECT theodb.graph_upsert('ws1','c1','chunk1',{})", quote(doc))).unwrap();
        let nodes2: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_nodes WHERE workspace_id='ws1'").unwrap().unwrap();
        let edges2: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_edges WHERE workspace_id='ws1'").unwrap().unwrap();
    ...
    fn m110_upsert_tenant_isolation() {
        Spi::get_one::<i64>("SELECT theodb.graph_upsert('wsA','c1','k','Alice knows Bob.')").unwrap();
        Spi::get_one::<i64>("SELECT theodb.graph_upsert('wsB','c1','k','Carol knows Dave.')").unwrap();
        let a: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_nodes WHERE workspace_id='wsA'").unwrap().unwrap();
        let b: i64 = Spi::get_one("SELECT count(*) FROM theodb.graph_nodes WHERE workspace_id='wsB'").unwrap().unwrap();
    ...
    fn m110_e2e_extract_to_expand() {
        ...
        Spi::get_one::<i64>("SELECT theodb.graph_upsert('e2e','c','k3','Carol met Dave.')").unwrap();
    ...
    fn m110_llm_path_fails_soft_to_heuristic() {
        ...
        assert_eq!(llm, vec!["acme corp", "alice", "bob", "paris"], "LLM path falls back to heuristic on unparseable reply");
    ...
    fn m110_bench_extraction_throughput() {
        ...
        for _ in 0..3 {
            ...
            total_edges = 0;
            for c in &chunks {
                let e: i64 = Spi::get_one(&format!("SELECT count(*) FROM ai.extract_graph({})", quote(c))).unwrap().unwrap();
                total_edges += e;
            }
        ...
        for (i, c) in chunks.iter().enumerate() {
            Spi::get_one::<i64>(&format!("SELECT theodb.graph_upsert('bench','c','k{i}',{})", quote(c))).unwrap();
        }
        ...
        assert!(extract_cps > 0.0 && total_edges > 0, "extraction produced edges at a positive rate");
    }
