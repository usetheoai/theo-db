# Synchronous per-row model HTTP calls (embed + ai.*), with batch/async deferred

- Status: proposed
- Date: 2026-06-29
- Deciders: TheoDB core (CTO sign-off pending)
...
- Add `theodb.embed_batch(text[])` mirroring `ai.generate_batch` — the embeddings endpoint
  already accepts an input array, so this is a low-effort, high-leverage mitigation that
  closes the embed N+1 (the most common bulk operation) with no design change.
...
- Bad: a column-wide call ties up one backend per row for the sum of per-row latencies, so
  `max_connections` (not CPU/RAM) is the first vertical wall under fan-out; a single
  transient HTTP failure aborts the whole statement (no retry today).
