# No embedding/chat result cache in v1 (YAGNI deferral)

- Status: proposed
- Date: 2026-06-29
- Deciders: TheoDB core (CTO sign-off pending)
- Tags: data-flow, cost, ai-surface, yagni
- Relates to: ADR 0002 (measurement-first), ADR-A (synchronous per-row model HTTP)
...
- Consistency with the `VOLATILE` per-call model (ADR-A): LLM/chat calls are treated as
