...
|---|---|---|---|---|---|---|
| Q1 | Como o pgvectorscale despacha entre tipos de storage (sbq/plain) de forma OCP — trait + enum, e onde o dispatch de versão/tipo é feito UMA vez? | techniques | `.claude/knowledge-base/references/pgvectorscale/pgvectorscale/src/access_method/` | `ast-grep run -p 'trait Storage { $$$ }' --lang rust` + `-p 'impl Storage for $T { $$$ }'` em `storage.rs`, `sbq/storage.rs`, `plain/storage.rs`; Grep `enum StorageType` + `impl StorageType` | Read `storage.rs:41` (trait), `:145-169` (enum + impl), e cada `impl Storage`; capturar como uma versão/tipo nova entra (add impl vs edit match) | Tabela: trait method → por-tipo impl → onde o dispatch resolve o tipo, com `path:line` |
...
| Q4 | Como o pgvectorscale testa que um scan sobre cada tipo de storage retorna o resultado correto — o padrão de teste que o M147 precisa para provar "comportamento preservado"? | tests | `.claude/knowledge-base/references/pgvectorscale/pgvectorscale/src/access_method/` | `ast-grep run -p '#[pg_test] fn $NAME() { $$$ }' --lang rust` em `sbq/tests.rs`, `plain/tests.rs`, `upgrade_test.rs` | Read cada `#[pg_test]` de scan/upgrade; capturar como asseveram recall/ordem e como testam compat de versão antiga | Tabela: teste → o que asservera → como monta o índice de cada versão, com `path:line` |
