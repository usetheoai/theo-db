...
| [Um benchmark que re-implementa o código mede uma cópia — ele passa enquanto a produção regride](benchmark-que-mede-uma-copia-do-codigo.md) | O bench do pgvectorscale re-implementa a estrutura de candidatos como cópia standalone, sem nenhum teste que assegure equivalência com a de produção. |
...
| [Uma contagem agregada credita ao fix unidades que falhavam por outra causa](contagem-agregada-mistura-classes-de-falha.md) | No baseline 100M do M169, só 1 das 6 falhas era o defeito que o milestone conserta — e ao classificar as outras 5 eu mesmo misturei classes, usando um discriminador que responde uma pergunta mais estreita do que eu supus. |
...
| [Corrigir a instância que mostraram, e deixar os irmãos vivos](corrigir-a-instancia-e-nao-a-classe.md) | Um revisor aponta UM caso; o fix fecha aquele caso e não varre os outros do mesmo formato — que continuam lá, agora com a falsa sensação de terem sido revisados. |
...
| [Extrapolar uma reta medida para um regime onde o MECANISMO é outro](extrapolar-reta-para-regime-de-outro-mecanismo.md) | A reta continua certa e a previsão tirada dela, não: previ por aritmética que a q32 não teria como completar (~9 GB de estado contra pool de 192 MiB) e ela completou em 290 s — sob streaming o DataFusion derrama para disco, e o pressuposto "o estado tem de caber em memória" morreu com a mudança de regime. |
...
| [Uma mensagem de erro genérica apaga a causa — e o bug reportado deixa de ser reproduzível](erro-generico-torna-o-bug-irreproduzivel.md) | O #132 dizia que o worker dead-letrava todo job; não reproduziu. O defeito real era o caminho de erro descartando a causa e um lote de zero linhas contado como sucesso. |
...
| [Destruir antes de provar a precondição deixa um estado PIOR que o inicial — e o vazio se disfarça de dado](destruir-antes-de-provar-a-precondicao.md) | O rebuild dropou hits_heap, recriou vazia, e só então descobriu que não conseguia ler o TSV. Onde não havia tabela passou a haver uma de 0 linhas — e 0 linhas lê como divergência de contagem, não como ausência. |
...
| [O sintoma nomeia a fase errada — o backtrace disse EXPLAIN onde o issue dizia planner](o-sintoma-nomeia-a-fase-errada.md) | O #135 reportava 18 min de hang no planner sobre tabela larga e propunha guard por largura; o gdb mostrou recursão infinita no deparse do EXPLAIN, e a mesma query EXECUTA em 0,537 s. |
