...
### D5 — Produce the `vector` return type via the `pgvector` Rust crate (MIT), not a `float4[]` cast
**Decision:** declare `theodb.embed(...) RETURNS vector` by returning the `pgvector` Rust crate's pgrx `Vector` type (`#[pg_extern] fn embed(...) -> pgvector::Vector`), so the SQL return is natively `vector` — no `float4[]` intermediary, no implicit-cast surprise. The crate is added to `theodb_rs/Cargo.toml` and audited in T5.1 alongside the HTTP crate.
**Rationale:** a bare `-> Vec<f32>` maps to SQL `float4[]`, which diverges from the public `theodb.embed → vector` contract (`sql/30`) and would break the parity oracle's `vector`-typed assertions (edge-case EC-1). `pgvector` is the canonical Rust binding for pgvector's `vector` type, MIT-licensed (D1-OK), with a `pgrx` feature. Rule 9 (don't hand-roll the type marshalling) + measurement of the real contract.
**Alternatives considered:** (a) return `Vec<f32>` and cast `float4[]::vector` in an SQL wrapper — rejected: an undocumented cast surface + an extra SQL shim (more moving parts than the typed crate); (b) hand-roll the `vector` send/recv in Rust — rejected: reinventing pgvector's binary protocol (Rule 9 + risk); (c) define the function in pure SQL calling out — rejected: defeats the purpose of the Rust rewrite.
**Consequences:** one more small MIT dep (audited in T5.1); the `theodb_rs` control keeps `requires = 'vector'` so the type resolves at install; no cast surface.
...
| Rust (cargo) | `pgvector` | `0.x` (pin exact in Cargo.lock) | MIT | Canonical Rust binding for pgvector's `vector` type (D5) — native `RETURNS vector`, no `float4[]` cast surface | `Vec<f32>` + SQL cast shim (undocumented cast); hand-rolled vector send/recv (reinvents pgvector protocol) |
