...
|---|---|---|
| **R0.1** | **Acervo local ANTES da web (Paulo, 2026-07-24).** O repositório mantém um acervo curado — **25 PDFs** em `.claude/knowledge-base/references/papers/` e **33 repos** de peers em `.claude/knowledge-base/references/`, indexados em `.claude/knowledge-base/references-catalog.md`. Antes de qualquer WebSearch/WebFetch, o agente **DEVE** verificar se a resposta já está no acervo (é mais barato, offline e já passou pelo gate de licença) e citar `arquivo:linha` / o PDF quando estiver. R0.1 **não substitui** R0: a web continua obrigatória para o que o acervo não cobre (SOTA posterior ao clone, blogs, releases). Ordem: **local → web → conhecimento interno (último recurso, declarado como tal)**. | Review-enforced + `CLAUDE.md § Acervo de referências` |
...
| R3.1 | **Instrument-validates-against-architecture (retro M162, item D).** A measurement DoD MUST verify its chosen instrument actually observes the thing measured, BEFORE the DoD is locked. Counter-example that cost a milestone's verdict a correction: the M162 plan named `shared_blks_read` as the I/O-vs-CPU oracle, but `theodb_columnar` decodes its stripes OUTSIDE PostgreSQL's shared-buffer manager, so that counter reads ~0 while the scan reads tens of GB from disk — the instrument could not answer the question. If no valid instrument exists for the architecture, say so (`INSTRUMENT-UNAVAILABLE`) and do NOT dress an inference as a measurement. | Review-enforced (this file); ties to `public-copy.md` (perf = claim, not opinion) |
