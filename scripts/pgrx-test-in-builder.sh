#!/usr/bin/env bash
# Roda cargo (build) OU cargo pgrx test do theodb_rs DENTRO do builder image (toolchain pgrx+pg18),
...
# Uso: scripts/pgrx-test-in-builder.sh [build --lib | pgrx test pg18 <filtro>]
set -euo pipefail
...
docker run --rm \
  ...
  bash -lc "
    ...
    # pgrx test faz 'cargo pgrx install' nos dirs de extensão do pg18 (root-owned) → torná-los graváveis
