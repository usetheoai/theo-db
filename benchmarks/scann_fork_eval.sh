...
# SAME seeded dataset and writes each one's recall x QPS so the fork/no-fork decision (docs/adr/0004) rests on
...
# Tunables (env): N (5000), DIM (32), NQ (100), K (10), RUNS (3), METRIC (cosine), SEED (14), OUT (docs/benchmarks)
set -euo pipefail
...
METRIC="${METRIC:-cosine}"; SEED="${SEED:-14}"; OUT="${OUT:-docs/benchmarks}"
