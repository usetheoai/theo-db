-- CH-benCHmark 22 analytical queries (Q1..Q22) — the standard CH-benCHmark analytical query set (DBTest 2011),
-- transcribed from the definitions at cmu-db/benchbase @ 33c00473807ebd49304d114a6d769d2d2b2bbb34 for the M130
-- OLAP result-consistency oracle (run_m130_htap.olap_result_consistency). These are the public benchmark query
-- definitions (like TPC-H queries), NOT BenchBase's Java code — used to assert each query returns a non-empty,
-- stable-shape result against TheoDB (the correctness oracle BenchBase itself lacks).

-- @Q1
SELECT ol_number, sum(ol_quantity) AS sum_qty, sum(ol_amount) AS sum_amount, avg(ol_quantity) AS avg_qty, avg(ol_amount) AS avg_amount, count(*) AS count_order FROM order_line WHERE ol_delivery_d > '2007-01-02 00:00:00.000000' GROUP BY ol_number ORDER BY ol_number;
...
-- @Q6
SELECT sum(ol_amount) AS revenue FROM order_line WHERE ol_delivery_d >= '1999-01-01 00:00:00.000000' AND ol_delivery_d < '2020-01-01 00:00:00.000000' AND ol_quantity BETWEEN 1 AND 100000;
...
-- @Q14
SELECT (100.00 * sum(CASE WHEN i_data LIKE 'PR%' THEN ol_amount ELSE 0 END) / (1 + sum(ol_amount))) AS promo_revenue FROM order_line, item WHERE ol_i_id = i_id AND ol_delivery_d >= '2007-01-02 00:00:00.000000' AND ol_delivery_d < '2020-01-02 00:00:00.000000';

-- @Q15 (view rewritten as an equivalent CTE — single statement, no leftover view; semantically identical)
WITH revenue0 (supplier_no, total_revenue) AS (SELECT mod((s_w_id * s_i_id),10000) as supplier_no, sum(ol_amount) as total_revenue FROM order_line, stock WHERE ol_i_id = s_i_id AND ol_supply_w_id = s_w_id AND ol_delivery_d >= '2007-01-02 00:00:00.000000' GROUP BY supplier_no) SELECT su_suppkey, su_name, su_address, su_phone, total_revenue FROM supplier, revenue0 WHERE su_suppkey = supplier_no AND total_revenue = (select max(total_revenue) from revenue0) ORDER BY su_suppkey;
-- @Q16
