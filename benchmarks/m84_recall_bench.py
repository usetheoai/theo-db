...
def read_ivecs(path, limit):
    ...
    return out
N=int(os.environ.get('M84_N','1000000')); NQ=int(os.environ.get('M84_NQ','200')); LISTS=int(os.environ.get('M84_LISTS','500'))
out=open('/home/theo/m84b_out.txt','a')
...
conn=psycopg2.connect(host='localhost', port=PORT, dbname='m84', user='theo'); conn.autocommit=True
cur=conn.cursor()
def load(tbl):
...
emit(f"M84_LOADED n={len(base)} nq={NQ} lists={LISTS}")
...
emit(f"M84_BUILD v5_s={build('sift5','sift5_idx',f' WITH (lists={LISTS}, pq_subspaces=32, aq_threshold=2000, separate_storage=1)'):.1f}")
emit(f"M84_BUILD v4_s={build('sift4','sift4_idx',f' WITH (lists={LISTS}, pq_subspaces=32, aq_threshold=2000)'):.1f}")
...
for of in [1,8,32]:
    for pr in [16,32,64,128,256]:
        r5,q5=rq("sift5",pr,of); r4,q4=rq("sift4",pr,of)
        sp=q5/q4 if q4>0 else 0
        emit(f"M84_RESULT of={of} probes={pr} v5_recall={r5:.4f} v5_qps={q5:.1f} v4_recall={r4:.4f} v4_qps={q4:.1f} speedup={sp:.2f}")
emit("M84_DONE")
