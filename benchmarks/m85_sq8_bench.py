...
def read_ivecs(path, limit):
    ...
    return out
N=int(os.environ.get('M85_N','1000000')); NQ=int(os.environ.get('M85_NQ','200')); LISTS=int(os.environ.get('M85_LISTS','500'))
out=open('/home/theo/m85_out.txt','a')
...
conn=psycopg2.connect(host='localhost', port=PORT, dbname='m85', user='theo'); conn.autocommit=True
cur=conn.cursor()
def load(tbl):
...
load("sift6"); load("sift5")
emit(f"M85_LOADED n={len(base)} nq={NQ} lists={LISTS}")
...
emit(f"M85_BUILD v6sq8_s={build('sift6','sift6_idx',f' WITH (lists={LISTS}, pq_subspaces=32, aq_threshold=2000, separate_storage=1, refine=1)'):.1f}")
emit(f"M85_BUILD v5f32_s={build('sift5','sift5_idx',f' WITH (lists={LISTS}, pq_subspaces=32, aq_threshold=2000, separate_storage=1)'):.1f}")
for nm in ("sift6_idx","sift5_idx"):
    cur.execute(f"SELECT pg_size_pretty(pg_relation_size('{nm}'))"); emit(f"M85_SIZE {nm}={cur.fetchone()[0]}")
...
for of in [8,16,32]:
    for pr in [32,64,128]:
        r6,q6=rq("sift6",pr,of); r5,q5=rq("sift5",pr,of)
        sp=q6/q5 if q5>0 else 0
        emit(f"M85_RESULT of={of} probes={pr} v6_recall={r6:.4f} v6_qps={q6:.1f} v5_recall={r5:.4f} v5_qps={q5:.1f} qps_ratio={sp:.2f} recall_loss={r5-r6:.4f}")
emit("M85_DONE")
