...
- **`SELECT count(*)` retornava 0 num índice vetorial — resposta errada, sem erro.** Um scan sem `ORDER BY <->` saía silenciosamente do `amrescan`, o heap de candidatos ficava vazio e o executor concluía que a tabela não tem linhas. Medido em PG 18.4 com `enable_seqscan=off` numa tabela de 500 linhas: B-tree devolvia 500, o índice `theodb_hnsw` devolvia **0**, com plano `Index Only Scan` e nada no log. Agora falha alto com erro tipado e acionável, como o `pg_vector` faz há anos (`hnswscan.c:214`). Defeito **pré-existente**, não introduzido por este milestone (M146, #177)
