...
### Changed
- **M57 — `HNSW_M` 16→32 no índice AM (recall gate ≥0.99)**: a conectividade do grafo (`M`) é o lever canônico de recall do HNSW. Bissectou-se que o teto de recall antigo (~0.96–0.974) é conectividade base (não contenção paralela — igual no build sequencial). `m=32` dobra a conectividade; page-safe (`nbr_size(32)=6532 B < BLCKSZ`), `m0=64` (auto). Custo: build/índice ~2× (aceito — Esforço ≠ Complexidade). Só o índice AM. Ver `docs/benchmarks/m57-sbq-superiority.md`.
...
### Fixed
- **M57 — investigação do teto de recall do build paralelo do HNSW (duas tentativas de fix REFUTADAS por medição, honesto)**: o `insert_node` paralelo (`ann/hnsw_parallel.rs`) satura em recall@10 ~0.974 a 500k×768d (< gate 0.99). Tentou-se (1) `ef_construction` 64→200 → PIOROU (0.832); (2) o "minimal fix" MERGE (extend+prune dos back-links) que o comentário do código previa → PIOROU (0.846, recall não-monotônico em ef_search = grafo corrompido). Ambos revertidos (config OVERWRITE + efc=64 mantida, a melhor medida). Conclusão: o teto NÃO é o lost-update nem o efc; a causa-raiz do recall do build paralelo continua não-isolada e exige uma investigação de qualidade-de-grafo própria (M60-class), fora do escopo do M57. O veredito SBQ (0.31–0.77×, falsificado) é robusto ao recall casado. Ver `docs/benchmarks/m57-sbq-superiority.md` § caveat 3 + `docs/adr/0018`.
