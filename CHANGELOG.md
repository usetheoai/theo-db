...
### Changed
- **M56 fase 2 — slot-reuse é OFF por default (achado do benchmark, Regra 3):** o benchmark de churn (`docs/benchmarks/m56-slot-reuse-churn.md`) mediu que o slot-reuse **suprime o gatilho de compactação** (consome os tombstones antes do threshold → o fold que REPARA o grafo nunca dispara) e o **recall@10 despenca (~0.57 vs ~0.95** do caminho default navigate-through+fold). Slot-reuse limita o TAMANHO do índice (0% vs 284% de crescimento) mas troca isso por RECALL — troca ruim. Por isso `theodb.hnsw_slot_reuse` **default OFF** (opt-in); o navigate-through + fold-compaction (DoD 2/3, recall estável) permanece o default correto.
