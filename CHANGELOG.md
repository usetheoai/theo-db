...
- **M56 DoD-1 fase 2 (RepairGraph / in-place insert) — fundação**: `hnsw_page::find_reusable_slot` acha um slot de element tombstonado reusável para um novo nó (entrada do slot-reuse no `aminsert`, padrão `pgvector/hnswinsert.c`); `page::read_all_page_items_with_off` expõe o `OffsetNumber` de cada item. Design completo do subsistema (search+link on-disk, crash-safe, ADRs de level-matching e ordenação) em `knowledge-base/plans/m56-repairgraph-inplace-insert-plan.md`. Ainda NÃO wired no `aminsert` (restam T1.2/T1.3/T2.1/T3/T4).
