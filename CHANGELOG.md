...
- Busca lexical BM25 disponível na instalação padrão, e medida: em corpus público com julgamento humano (BEIR SciFact, 300 consultas) ela entrega **mais que o dobro** da qualidade de recuperação do `ts_rank_cd` nativo do Postgres — nDCG@10 de 0,63 contra 0,30. Até aqui o pilar existia, era medido e era robusto, mas ficava fora do binário entregue — quem instalava o TheoDB não tinha `bm25_build`/`bm25_search`. Passa a acompanhar a extensão sem nenhuma flag de compilação. As funções de andaime do spike (`lexical_spike_*`) continuam fora da superfície pública, por opção. (#M186)
