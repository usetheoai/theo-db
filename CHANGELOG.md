...
### Fixed
- **M57 — qualidade do build paralelo do HNSW: MERGE em vez de OVERWRITE dos vizinhos (recall a escala)**: o `insert_node` paralelo (`ann/hnsw_parallel.rs`) sobrescrevia `node[layer]` com os vizinhos selecionados, clobberando back-links que outras threads já haviam inserido (lost-update lógico sob contenção). O M57 MEDIU que isso degrada o recall a escala (teto ~0.974 a 500k×768d) e PIORA com `ef_construction` maior (0.974→0.832 a efc=200: mais candidatos → mais back-links clobberados). Fix (o "minimal fix" que o próprio comentário do código previa como "YAGNI until measured", agora medido): mesclar os back-links in-flight com os selecionados, deduplicar e podar ao `m_layer` com a heurística de diversidade. Guardado pelo recall-parity gate do build paralelo. Ver `docs/benchmarks/m57-sbq-superiority.md` + `docs/adr/0018`.
