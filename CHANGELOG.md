...
### Added
- **M60 (investigação, HONEST-NEGATIVE) — medição decisiva do recall do HNSW próprio vs pgvector a 500k×768d** (`docs/benchmarks/m60-hnsw-recall.md`, `docs/benchmarks/m60-raw/`, `benchmarks/run_m60_recall.py` (NEW), `benchmarks/run_m60_pgvector_control.py` (NEW), blueprint `m60-hnsw-recall-quality`): head-to-head no MESMO corpus gaussian-mixture (droplet c-8, pg17) — pgvector best recall@10 = **0.988**, theodb_hnsw f32 = 0.974, theodb SBQ (over_fetch=32) = **0.986**. Dois achados (Regra 3): (1) **o gate 0.99 é artefato do dado** — o próprio pgvector só chega a 0.988 (256 clusters apertados em 768d → teto de recall@10 < 0.99 para índices HNSW); a DoD do M60 deve virar **paridade-pgvector**, não 0.99 absoluto; (2) existe um gap real **~1.4pt** (f32 vs pgvector), com o SBQ já em quase-paridade. A hipótese de fix #1 do discover (descida de build por beam ef=1) foi **implementada e REFUTADA por medição** (no-op: `search_layer(ef=1)` ≡ hill-climb; a descida do pgvector também é ef=1) — revertida. **M60 permanece ABERTO** (próximo lever: `select_from` keep-back ordering). Risco ALTO confirmado empiricamente.
...
### Changed
- **M60 CONCLUÍDO — DoD de recall reenquadrada para PARIDADE-pgvector (ADR-0030), fechado pelo caminho SBQ** (`docs/adr/0030-m60-recall-parity-not-absolute-099.md` (NEW), `ROADMAP.md § M60` [x]): a medição head-to-head no mesmo corpus a 500k×768d provou que o gate `recall@10 ≥0.99` é **artefato do dado** — o próprio pgvector só chega a **0.988** (256 clusters apertados em 768d ⇒ teto de recall@10 < 0.99 para índices HNSW). A DoD passa a ser **paridade-pgvector** (measurement-first, alinhado ao North Star ADR-0002). **Paridade atingida pelo caminho SBQ: 0.986 ≈ 0.988** (theodb over_fetch=32 rerank vs pgvector), medida com GT exato. O gap do f32 puro (0.974, ~1.4pt) fica como **follow-up autorizado** (opção B) — resistiu a 5 levers refutados por medição (efc↑, MERGE, m↑, descida-beam ef=1, multi-entry `ep←W`). Sem claim de superioridade (é paridade de recall; a latência/QPS é escopo do M71, que herda o +29% de QPS medido do grafo multi-entry). ADR-0030.
