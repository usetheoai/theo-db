...
### Changed
- **M57 — achado de qualidade do grafo HNSW (medido, honesto)**: o grafo do `theodb_hnsw` satura em recall@10 ~0.974 a 500k×768d (abaixo do gate 0.99 mesmo a ef_search=1000, o máximo). Tentou-se elevar `ef_construction` 64→200 para cruzar o gate, mas **DEGRADOU** o recall (→0.832, medido em 3 runs) — revelando que o teto NÃO é a profundidade de construção e sim um **gap no heurístico de seleção de vizinhos** (o theodb não aplica a poda por diversidade do HNSW/pgvector). Config mantida em efc=64 (a melhor). Cruzar 0.99 exige implementar a poda por diversidade — milestone de qualidade-de-grafo próprio (FORA do escopo do M57, que mede o SBQ). Ver `docs/adr/0018` + `docs/benchmarks/m57-sbq-superiority.md` (§ caveat 3).
