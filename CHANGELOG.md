...
### Added
- **M59 (P1) — Fase 5: veredito D3 (HONEST-NEGATIVE com achado mecânico)** (`docs/benchmarks/m59-anisotropic-ah.md` + `docs/benchmarks/m59-raw/`, ADR `docs/adr/0019`): o índice anisotrópico-PQ+AH está implementado e validado por correção (175 pg_tests), mas a 500k×768d fica em **paridade** com o f32 (1.01× in-RAM, 1.03× sob pressão de RAM 1.3GB) — **não fecha o gap ~25× vs ScaNN**. Mecanismo medido: (1) o HNSW tem localidade de acesso → o f32 não thrasha sob pressão → os códigos AQ de 4 bytes (768× menores) não compram I/O; (2) o kernel AH **batched** (4.75× micro-bench) precisa de candidatos **contíguos** para o `pshufb`, mas a caminhada HNSW é pointer-chasing (nó-a-nó) → só o `ah_score` single-code roda (mais lento) → o ganho SIMD fica **inacessível no carrier HNSW** — exatamente o risco ADR-D4 do blueprint, agora medido. Sinal positivo: a 20k in-RAM o AQ **vence o f32 (1.16×)** — o 1º índice quantizado do theodb a superar o f32 (o SBQ do M57 era 0.35–0.77×), provando que o lever AH-LUT é o certo. Decisão (ADR-0019): o eixo algorítmico está resolvido; o ganho exige um **carrier IVF de batch-scan** (próximo milestone, motivado por medição — o que ScaNN/FAISS usam). Driver `benchmarks/run_m59_pressure.py` (split build/measure, pressão via `docker --memory`, reusa o harness M57 — Rule 9).
