...
### Added
- **M62 — Fase 3: benchmark HTAP de 3 eixos + ADR-0021** (`docs/benchmarks/m62-htap.{md,json}`, `docs/benchmarks/m62-raw/`, `docs/adr/0021-m62-htap-codegen-surface.md`): o HTAP lakehouse-materializado do TheoDB medido a 5M — **eixo-1 OLAP colunar ~31×** (olap_sql/read_parquet 15.9ms vs heap GROUP BY 492.9ms, checksum-matched); **eixo-2 custo de refresh ~1.2s** (COPY row→Parquet, o preço da freshness); **eixo-3 não-interferência: OLTP NÃO degrada sob OLAP concorrente** (snapshot Parquet read-only). Veredito: funciona e entrega o ganho sem impactar o OLTP, ao preço de refresh explícito + freshness datada + storage 2×. ADR-0021 registra o pivô codegen (pg_duckdb proíbe DuckDB em função) + as 5 alternativas rejeitadas (funções-executam-DuckDB, background worker, MotherDuck, moonlink-BSL, só-doc). Honesto: HTAP assistido, não single-call transparente nem in-memory do AlloyDB (aposta D2).
- **M62 — implement Fases 1-2: superfície HTAP codegen (16 pytest GREEN)** (`sql/85-theodb-htap.sql`, `Dockerfile`, `benchmarks/tests/test_htap.py`): **achado arquitetural medido** — o pg_duckdb PROÍBE execução DuckDB dentro de funções plpgsql (`DuckDB execution is not supported inside functions`; sem GUC p/ permitir). A abordagem função-que-chama-duckdb.query foi refutada e **pivotada honestamente para codegen**: `theodb.htap_refresh_sql`/`theodb.olap_sql` retornam o SQL (COPY/read_parquet) que o cliente roda no nível de conexão (onde o pg_duckdb permite DuckDB), `theodb.htap_register`/`theodb.htap_freshness` são SQL puro (catálogo `theodb._htap_snapshots`). NENHUMA função chama duckdb.query internamente. Dir de snapshot `/var/lib/postgresql/htap` criado no build. **16 pytest GREEN** contra a imagem: fluxo COPY→Parquet→register→olap checksum-matched vs heap fresco, freshness/staleness explícita, race-aware OLTP-sob-OLAP, negativos tipados. Honesto: NÃO é HTAP single-call transparente (pg_duckdb não permite); é fluxo statement-level assistido (aposta lakehouse/D2).
- **M62 — implement Fases 1-2: superfície HTAP `theodb.htap_refresh`/`olap`/`htap_freshness` (own-code SQL)** (`sql/85-theodb-htap.sql`, `sql/theodb--1.3--1.4.sql`, `theodb.control`, `Dockerfile`, `benchmarks/theodb_bench/db.py`, `benchmarks/run_m62_htap.py`, `benchmarks/tests/test_htap.py`): 3 funções plpgsql próprias (ADR-0021 D2 — plpgsql sobre Rust, parsimony ladder) que compõem 100% sobre o pg_duckdb embarcado (M61/ADR-0020, MIT — Regra 9, zero peça nova). `theodb.htap_refresh(regclass)` materializa a tabela row para um snapshot Parquet datado (`COPY … TO … (FORMAT parquet)`) e registra `(rel, path, now())` no catálogo `theodb._htap_snapshots` (upsert, latest-wins); retorna o timestamp. `theodb.olap(regclass)` roteia a agregação para o snapshot colunar via `read_parquet` dentro de `duckdb.query` (a sintaxe confirmada no M61) e retorna `{"snapshot_at": ts, "rows": [...]}` — as linhas refletem o SNAPSHOT (stale-by-design; **freshness datada e explícita**, não HTAP-transparente). `theodb.htap_freshness(regclass)` retorna o lag (`now() - refreshed_at`). **Fail-closed tipado** (Regra 8): sem snapshot ⇒ erro `P0002` com "call theodb.htap_refresh first" (nunca NULL silencioso); Parquet ausente/corrompido ⇒ erro io_error claro. Dynamic SQL injection-safe (regclass + `%I`/`%L`). Wired na extensão (concat `sql/85` no `theodb--1.0.sql` + migration `1.3→1.4`, `default_version` 1.4). **Teste race-aware (T2.1):** `run_m62_htap.py::measure_mixed_load` usa `threading.Barrier(2)` para GARANTIR sobreposição INSERTs OLTP × `theodb.olap` OLAP e assere degradação p95 ≤ fator + overlap confirmado (não pode passar sequencialmente); teste de snapshot consistente sob INSERTs concorrentes. 13 testes de integração (round-trip checksum-matched via `_results_match` — Regra 9; freshness/staleness; fallback `force_execution` fresco; 3 negativos tipados). **Validado em `implement`:** DDL parseia em postgres:17 real, funções resolvem, erros tipados P0002 disparam, aritmética de lag correta, idempotência (CREATE OR REPLACE/IF NOT EXISTS), skips limpos sem pg_duckdb (13/13 SKIP, sem falso-verde), ruff zero-warning. **Resta (droplet):** Fase 3 (benchmark de 3 eixos → `docs/benchmarks/m62-htap.{md,json}`) + Fase 4 (build da imagem + suíte contra o container + ADR-0021) — o COPY→Parquet + read_parquet de dentro de plpgsql (Q2 do plano) só valida end-to-end com a imagem pg_duckdb.
- **M62 — plan: superfície HTAP unificada** (`.claude/knowledge-base/plans/m62-htap-unified-surface-plan.md`): 4 fases/6 tasks TDD (funções `theodb.htap_refresh`/`theodb.olap`/`theodb.htap_freshness` own-code SQL em `sql/85-theodb-htap.sql` → carga mista OLTP↔OLAP race-aware → benchmark de 3 eixos → integration). 2 ADRs (lakehouse-materializado vs 5 alternativas incl. moonlink-BSL/MotherDuck-SaaS/AGPL; SQL/plpgsql vs Rust). ADR formal = 0021. Coverage 100%. Gate SHIPPABLE_WITH_CAVEATS (89.0). Honesto: fluxo row↔colunar com freshness datada, não HTAP-transparente.
...
### Fixed
- **M62 — pivô CODEGEN: `pg_duckdb PROÍBE DuckDB dentro de função`, a superfície HTAP vira statement-level** (`sql/85-theodb-htap.sql`, `sql/theodb--1.3--1.4.sql`, `benchmarks/run_m62_htap.py`, `benchmarks/tests/test_htap.py`): BLOQUEADOR arquitetural **medido** (reproduzível no container `theodb:m62`) — `SELECT … FROM duckdb.query(…)` / `COPY … TO … (FORMAT parquet)` / `read_parquet(…)` de dentro de uma função dispara `ERROR: DuckDB execution is not supported inside functions` (sem GUC para relaxar). Isso invalidava as Fases 1-2 (as funções `theodb.htap_refresh`/`theodb.olap` chamavam `duckdb.query`/`COPY` INTERNAMENTE — sempre falhavam). No nível de STATEMENT (conexão, fora de função) o `COPY`/`duckdb.query`/`read_parquet` FUNCIONAM (provado no M61). Pivô honesto (não é workaround — é o design correto dada a restrição): as funções viram **geradoras de SQL (codegen) + catálogo puro**, NENHUMA chama `duckdb.query` internamente. `theodb.htap_refresh_sql(regclass)→text` retorna o statement `COPY (SELECT * FROM tbl) TO '<path>' (FORMAT parquet)` (string building `format()`+`%L`, injection-safe; NÃO executa) — o cliente roda no nível de conexão. `theodb.htap_register(regclass, text)→timestamptz` faz upsert no catálogo `theodb._htap_snapshots` (SQL puro, sem DuckDB — roda em função); o cliente chama DEPOIS de rodar o COPY. `theodb.olap_sql(regclass)→text` lê o path do catálogo e retorna `SELECT * FROM duckdb.query($$ SELECT category, count(*) c, round(avg(amount),4) a FROM read_parquet('<path>') GROUP BY category ORDER BY category $$)` (**`SELECT *`** — colunas nomeadas quebram o record do `duckdb.query`, medido) — o cliente roda no nível de conexão; sem snapshot ⇒ erro tipado `P0002` "call theodb.htap_refresh_sql + run it + theodb.htap_register first". `theodb.htap_freshness(regclass)→interval` = `now()-refreshed_at` (SQL puro). Migration `1.3→1.4` faz `DROP FUNCTION IF EXISTS` das funções antigas quebradas + cria as novas (idempotente; no-op no greenfield). Testes reescritos para o padrão statement-level: `test_flow_roundtrip` (refresh_sql→cliente roda COPY→register→olap_sql→cliente roda→checksum-match vs heap fresco via `_results_match`), `test_freshness_reflects_lag_and_olap_is_stale_until_refresh` (lag cresce + snapshot antigo), **race-aware (T2.1)** (`measure_mixed_load` com `threading.Barrier(2)` — OLAP statement-level concorrente com INSERTs OLTP, snapshot Parquet read-only ⇒ leitura consistente; overlap confirmado, não-degradação p95), 4 negativos tipados (olap_sql/freshness sem snapshot ⇒ P0002; register de tabela inexistente ⇒ 42P01; path vazio ⇒ 22023). Os statements DuckDB rodam no nível de conexão (psycopg2 autocommit), NUNCA dentro de função. **Validado local (postgres:17 sem pg_duckdb):** codegen retorna as strings exatas, catálogo/register/freshness funcionam, 4 negativos tipados PASS, 11 testes que tocam DuckDB SKIP limpo (sem falso-verde), greenfield+upgrade idempotentes (funções antigas DROP, 5 novas resolvem), ruff zero-warning. **Resta (droplet `theodb:m62`):** execução real do COPY→Parquet + `read_parquet` end-to-end (só valida na imagem com pg_duckdb) + Fase 3 (benchmark) + Fase 4 (ADR-0021).

### Security
## [0.51.0] - 2026-07-09

### Added
- **M61 — implement Fase 2+3: smoke CI + benchmark de adoção honesto** (`scripts/m61-pgduckdb-smoke.sh`, `benchmarks/run_m61_columnar_adoption.py`, `docs/benchmarks/m61-columnar-adoption.{md,json}`, `docs/adr/0020-m61-embed-pgduckdb.md`): smoke `test_pg_duckdb_analytic_query_plans_under_duckdb` PASSA (preload + 6 extensões coexistem + duckdb.query→42 + community_ext=off + query analítica planeja sob DuckDB + índice vetorial junto). Benchmark de adoção (5M, ≥3 runs mean±std, checksum numérico correto): **sobre HEAP row-store o force_execution do DuckDB PERDE** (0.63–0.89×, honest-negative — ler row-format via DuckDB adiciona overhead); **sobre PARQUET colunar o DuckDB VENCE ~9× a 5M** (1.56×→8.78×) — o ganho materializa em dados colunares (na faixa do 14× do mooncake). Ceticismo de medição pegou um artefato (700× era erro de sintaxe read_parquet, não real). Valor entregue: analytics colunar sobre arquivos (data-lake/lakehouse, aposta D2), NÃO acelerador transparente do heap. Licença MIT (D1 ✓).
- **M61 — implement Fase 1+2: pg_duckdb embarcado (columnar/HTAP) na imagem** (`Dockerfile`): novo estágio `pgduckdb-builder` (pg_duckdb v1.1.1 MIT, `DUCKDB_BUILD=ReleaseStatic` — bundle DuckDB estático) + COPY artifact-only + `shared_preload_libraries='pg_duckdb'` (append idempotente ao `postgresql.conf.sample`) + `CREATE EXTENSION pg_duckdb` no init. Imagem 813MB (+170MB do DuckDB, esperado). **Smoke verde**: as 6 extensões coexistem (vector+vectorscale+theodb+theodb_rs+pg_duckdb), `duckdb.query('SELECT 42')`→42, `force_execution` agrega sobre heap (o HTAP), índice `theodb_hnsw` funciona junto, `allow_community_extensions=off` (segurança). Dois fixes de build (risco #1): `libcurl4-openssl-dev` no builder (httpfs/cmake) + `libcurl4` no runtime (pg_duckdb.so linka libcurl dinâmico). **Achado honesto:** columnstore nativo persistente exige MotherDuck; o caminho HTAP é `force_execution` sobre o heap (analytics sem ETL). Benchmark de adoção (Fase 3) mede o pg_duckdb, não herda o 14× do mooncake.
- **M61 — plan: adoção columnar/HTAP (pg_duckdb)** (`.claude/knowledge-base/plans/m61-columnar-htap-adoption-plan.md`): 4 fases / 10 tasks TDD (build multi-stage pgduckdb-builder → smoke/gate de licença+CVE → benchmark de adoção columnstore-vs-heap → integration). 3 ADRs com alternativas (pg_duckdb vs pg_mooncake; static vs dynamic build; shared_preload append vs ALTER SYSTEM). Coverage 100%. Gate /plan-confidence: SHIPPABLE_WITH_CAVEATS (85.6). Honesto: vantagem analítica UNBENCHMARKED até a Fase 3 medir a superfície pg_duckdb.
- **M61 — discover: blueprint de adoção columnar/HTAP** (`.claude/knowledge-base/discoveries/blueprints/m61-columnar-htap-adoption-blueprint.md`): deep research R0 (11 fontes web citadas — repos pg_duckdb/pg_mooncake/DuckDB, LICENSEs, blog AlloyDB). Recomendação (ADR c/ alternativa): adotar **pg_duckdb** (MIT, GA v1.0, PG14–18 nativo) e NÃO pg_mooncake (camada sobre pg_duckdb, default PG18 — a rota que travou no ADR-0013). Build via multi-stage `pgduckdb-builder` + `shared_preload_libraries`. O 14× do M30 (medido no mooncake) marcado UNBENCHMARKED até o gate M61. Citus/Hydra AGPL barrados (D1).
...
### Added
- **M59 (P1) — Fase 5 cont.: layout v4 (código separado do f32) medido — honest-negative profundo**: implementado o v4 (element tuple HOT = só código AQ + `raw_addr`; f32 numa região RAW separada, rerank-only; byte-test prova que `ElementViewV4` não expõe `vec_bytes` — 177 pg_tests GREEN). Medido a 500k×768d sob pressão FORTE (`--memory=700m`, sb=128MB): **paridade** (AQ 2.3 vs f32 2.1 QPS, ambos colapsam ~2 QPS). Diagnóstico da curva: o rerank é barato (recall 0.958 já a over_fetch=4; p50 flat em over_fetch) → o WALK domina, e o v4 não o acelerou sob pressão — hipótese medível: interleaving das páginas hot/raw no `Packed` (poluição de cache no nível de página, apesar da separação no nível de código). Conclusão honesta (measurement-first): em NENHUMA config medida (v3, v4, in-RAM, pressão) o AQ superou o f32 em QPS a recall casado no carrier HNSW. Próximo lever registrado: page-ordering do v4, depois carrier IVF. Design: `knowledge-base/designs/m59-v4-code-vector-separation.md`.
- **M59 (P1) — layout v4: separação código/vetor (o fix real da paridade medida)** (`theodb_rs/src/am/hnsw_page.rs`, `build.rs`): implementa a correção primária do ADR-0019 (design note `.claude/knowledge-base/designs/m59-v4-code-vector-separation.md`). O layout v3-AQ co-localizava o código de 4B com o f32 de ~3KB no MESMO element tuple → ler o código paginava o tuple inteiro → o working set quente do walk continuava em ~1.5GB (@500k×768) → thrashava idêntico ao f32 sob pressão → paridade. O **v4 separa**: o element tuple HOT passa a carregar **só o código AQ + `nbr_addr` + `raw_addr`, SEM f32** (`~30B`, header `[E4_TAG][E4_LEVEL][E4_DELETED][E4_VERSION=4][E4_TID][E4_NBR_*][E4_RAW_*][E4_DIM][E4_CODE..]`); o f32 vai para uma **região raw FRIA separada** (raw-f32 tuple `[R_TAG][pad][f32]`, `RAW_TAG=3`), endereçada pelo `raw_addr` de cada element e lida **só no rerank** do top-`k·over_fetch` (uma leitura por sobrevivente). Body order da geração: `[hot elems][neighbors][codebook pages][raw f32 region]`, tudo posição-independente (`base`) — o fold relocável preserva ambos. A meta ganha a versão **v4** (`HNSW_STRUCT_VERSION_V4=4`) com descritor `[…AQ 13B…][raw_first:u32][raw_npages:u32]`; `pending_start` reserva a região raw. `decode_element_v4`/`encode_element_v4`/`decode_raw_vec`/`encode_raw_vec` novos; o scan (`traverse`/`load`) pontua o walk lendo **só o hot tuple** (`ah_score` sobre o código, o f32 NUNCA é paginado no walk) e o rerank segue `raw_addr` → `decode_raw_vec`; `enumerate_entries` (fold) segue `raw_addr` para reconstruir o corpus vivo. **Como nada foi RELEASED com o v3-AQ, o índice AQ foi redefinido para o v4** (v1/v2/SBQ inalterados byte-a-byte — backward-compat provada). Rule 8: código truncado ⇒ typed Err "REINDEX v4 corruption"; `raw_addr` inválido / tag errada ⇒ typed Err; `insert_inplace` recusa índices AQ/v4 (fallback pending). **TESTE DE BYTES (crítico, owner-requested):** `v4_hot_tuple_has_no_f32_walk_never_pages_the_vector` prova estruturalmente que o f32 saiu do hot-path — o hot tuple decodifica via `ElementViewV4`, que **não tem campo `vec_bytes`** (o f32 é inacessível pelo caminho de walk/score, só via `decode_raw_vec` numa tuple SEPARADA no rerank); pontuar um candidato com `raw_addr` envenenado (`u32::MAX`) sucede sem dereferenciar o f32. **pg_tests novos/adaptados GREEN**: `pack_v4_writes_codebook_hot_codes_and_separate_raw_region` (hot só-código + raw region separada + raw_addr válido + f32 round-trip), `v4_meta_roundtrips_raw_region_descriptor`, `v4_element_and_raw_tuple_roundtrip`, o teste de bytes, `aq_scan_matches_brute_knn_high_ef` (AH walk + rerank via raw_addr recupera recall@10 ≥ 0.9), `aq_scan_reads_flat_in_n`, `aq_index_survives_vacuum_fold`/`aq_fold_survives_concurrent_insert`/`aq_codebook_deterministic_under_parallel_build` (fold v4 via raw enumerate), dim=768 multi-página, + v1/v2/SBQ e ~175 pré-existentes sem regressão. **Restam**: re-benchmark a **2M×768** sob pressão real de RAM (a tese: hot ~250MB vs 6GB f32 ⇒ AQ v4 ≥ 2×) + review + release.
- **M59 (P1) — Fase 5: veredito D3 (HONEST-NEGATIVE com achado mecânico)** (`docs/benchmarks/m59-anisotropic-ah.md` + `docs/benchmarks/m59-raw/`, ADR `docs/adr/0019`): o índice anisotrópico-PQ+AH está implementado e validado por correção (175 pg_tests), mas a 500k×768d fica em **paridade** com o f32 (1.01× in-RAM, 1.03× sob pressão de RAM 1.3GB) — **não fecha o gap ~25× vs ScaNN**. Causa-raiz validada por matemática: **o layout v3 co-localiza o código AQ com o vetor f32 no mesmo element tuple** (`[f32 3072B][código 4B]`), então ler o código de 4B pagina o tuple de ~3KB → o working set quente do AQ é o MESMO do f32 (1.5GB @500k), não encolhe → thrasha idêntico sob pressão → paridade (a conta bate com a medição). O scoring NÃO é o gargalo (AH single-code ~100× mais barato que cosine f32); o page-read/working-set é. ScaNN/FAISS vencem porque guardam SÓ os códigos no hot-path (f32 separado, rerank-only). Sinal positivo (ruído de 1 run/20q): a 20k in-RAM o AQ mede 1.16× (índice inteiro cabe em cache por acidente de escala). Decisão (ADR-0019): o eixo algorítmico está correto; o fix real é de **LAYOUT — separar o código do f32** (v4: element tuple = só código; f32 rerank-only → hot ~50MB vs 1.5GB), não a troca de carrier. Driver `benchmarks/run_m59_pressure.py` (reusa o harness M57 — Rule 9).
- **M59 (P1) — Fase 5: harness de benchmark AQ v3** (`benchmarks/run_m59_aq.py`): reusa o harness M57 (`_make_dataset` gaussian-mixture, `_queries`, `_ground_truth`, `_measure`, `_conn` com keepalives — Rule 9) e adiciona só a spec `theodb_hnsw_aq` (`WITH (pq_subspaces=M, pq_bits=4, aq_threshold=η×1000)`) vs SBQ (v2) vs f32 (v1) vs pgvector, cosine. Mede recall@10 + QPS@recall≥0.99 (AQ/f32 e AQ/pgvector ratios) para o veredito D3. `over_fetch` é o knob varrido (recall-recovery). Smoke a 20k pegou 2 bugs reais (aq_threshold milli-scale; **codebook multi-página PageAddItem a dim=768** — corrigido em § Fixed).
- **M59 (P1) — implement Fase 4 (T4.1/T4.2): branch AH no scan v3 (LUT once + walk AH + rerank f32)** (`theodb_rs/src/am/hnsw_page.rs`): o `traverse` do `theodb_hnsw` ganha o caminho AQ — para um índice **v3** (`meta.aq_m>0`) ele reconstrói o `AqQuantizer` do codebook persistido e pré-computa a **`Lut16` UMA vez por query** (`vec::ah::build_lut16`, Fase 2), então o walk pontua cada candidato pelo AH quase-grátis `Σ LUT[i][code_i]` sobre os códigos 4-bit inline (`ah_score`, sem multiply f32, sem decode) em vez do f32/Hamming. Os sobreviventes do walk (pool alargado pelo GUC `over_fetch` reusado — parsimony rung-4, sem GUC novo) são **reranqueados por f32 exato** (padrão de recall-recovery do ADR-0018, reusado verbatim do caminho SBQ). O scan ramifica por `(lut, qcode)`: `Some(lut)` ⇒ AH (v3), `Some(qcode)` ⇒ Hamming (v2/SBQ), `(None,None)` ⇒ f32 exato (v1) — byte-idêntico para v1/v2, o seam `NeighborSource` (`scan_core.rs`) **não muda** (D1). Guard de comprimento Rule 8: um código AQ truncado on-disk ⇒ typed Err ("REINDEX v3 corruption"), nunca score errado nem panic. **3 pg_tests novos GREEN**: `aq_scan_matches_brute_knn_high_ef` (v3 real-graph, AH-walk+rerank recupera **recall@10 ≥ 0.9** vs o oracle seqscan, over_fetch=6), `aq_scan_truncated_code_is_typed_err` (código curto ⇒ Err tipado, negativo), `aq_scan_reads_flat_in_n` (scan v3 retorna top-k limitado via Index Scan, O(ef·M) flat em N, métrica wiring-triad `THEODB_SCAN_PROFILE`). Suíte total **174 passed / 0 failed** no builder pg17 (171 pré-existentes + 3, zero regressão). Fase 5 (benchmark D3 recall×QPS em SIFT1M real vs SBQ/ScaNN) + review + release restam.
- **M59 (P1) — implement Fase 3 (T3.3): wiring build+fold persiste o AQ v3** (`theodb_rs/src/am/build.rs`, `options.rs`, `aq.rs`): `ambuild_hnsw` agora escolhe o layout persistido pelos reloptions — `WITH (pq_subspaces=M)` (M>0) treina o quantizer anisotrópico (Fase 1) e empacota **v3** via `pack_aq` (helper `pack_hnsw_for_build`); sem reloption o caminho é EXATAMENTE o de hoje (v1/v2 byte-idêntico, provado por `ambuild_without_reloption_stays_v1`). O **fold do VACUUM** (`vacuum_rebuild_hnsw_structured`) preserva o v3 através da compactação: novo `pack_fold_layout` lê o discriminador **da meta persistida** (não do reloption — `aq_m>0` ⇒ recupera `bits`/`η` do codebook via `AqQuantizer::from_meta_bytes` e re-treina AQ sobre os vetores vivos; senão preserva SBQ/v1 via `sbq_bits`), reusando o mesmo pack posição-independente + o lock advisory EXCLUSIVE existente (M48/M56) — **nenhum lock novo**. Novo accessor `AqQuantizer::bits()`; removidos os `#[allow(dead_code)]` de `pack_aq` + dos 3 resolvers `*_from_relation` (wiring triad pillar-a satisfeito). **6 pg_tests novos GREEN**: build v3 (aq_m/codebook/η do reloption + scan f32 exato), não-regressão v1 sem reloption, `aq_index_survives_vacuum_fold` (fold re-treina o codebook, mantém v3, scan correto, nenhum id deletado sobrevive), **RACE-1** `aq_codebook_deterministic_under_parallel_build` (codebook AQ byte-idêntico sequencial-vs-paralelo — sem data race no drain), **RACE-2** `aq_fold_survives_concurrent_insert` (fold + insert serializados pelo lock existente preservam v3 + acham a linha), e negativo `ambuild_v3_rejects_indivisible_dim` (dim%m≠0 ⇒ typed Err no build, Rule 8). Suíte total **171 passed / 0 failed** no builder pg17 (165 pré-existentes + 6). Fases 4-5 (branch AH no scan v3 + benchmark D3) restam.
- **M59 (P1) — implement Fase 3 (T3.1): meta layout v3 codec + AQ code trailing no element tuple** (`theodb_rs/src/am/hnsw_page.rs`): a meta page do `theodb_hnsw` ganha a versão **v3** (`HNSW_STRUCT_VERSION_AQ=3`), que serializa o codebook AQ (`AqQuantizer::to_meta_bytes`, Fase 1) num trailer `[aq_m:u8][cb_len:u32][aq_codebook]` — espelho exato do trailer v2/SBQ (helper `decode_trailer` compartilhado, DRY). O **byte de versão discrimina** v1 (f32-only, 45 bytes) / v2 (SBQ) / v3 (AQ); AQ e SBQ são mutuamente exclusivos por índice (D1). Cada element tuple carrega o código AQ (`⌈m/2⌉` bytes) trailing após o vetor f32, reusando o slot `code_bytes` que `decode_element`/`elem_size` já parametrizam (zero mudança no read path). Novo `pack_aq(idx, base, m, bits, aq_threshold)` treina o quantizer + escreve os códigos inline; `pack_at`/`pack_sbq` roteiam por um core `pack_kind` com enum `CodeKind` (None/Sbq/Aq) e seed de build fixa (fold determinístico). **BACKWARD-COMPAT crítica preservada**: ler meta v1/v2 continua bit-exato (WAL/crash-safety) — provado por `v1_v2_meta_still_decodes` + todos os testes SBQ/v1/v2 pré-existentes seguem verdes. **6 pg_tests novos GREEN** (roundtrip v3, backward-compat v1/v2, `pack_aq` código==encode, m=0⇒v1 idêntico, versão desconhecida/trailer truncado⇒typed Err, element tuple roundtrip+size) — suíte total **165 passed / 0 failed** no builder pg17. Caller de build (T3.3) + scan (Fase 4) restam (deferidos com `#[allow(dead_code)]` honesto em `pack_aq`).
- **M59 (P1) — implement Fase 3 (T3.2): reloption AQ (`pq_subspaces`/`pq_bits`/`aq_threshold`)**: `theodb_rs/src/am/options.rs` ganha os 3 build-knobs do AQ para o `theodb_hnsw` — `WITH (pq_subspaces=M, pq_bits=4, aq_threshold=T)` — no mesmo padrão pgrx `build_reloptions` do `sbq_bits`/`lists`. `pq_subspaces` default **0 = AQ off** → índice v1/v2 byte-idêntico (backward-compat). `aq_threshold` (η) armazenado milli-scaled (η×1000) para caber num int reloption (KISS). Resolvers `*_from_relation` com clamp/defaults defensivos + safety docs (Rule 8). Compila limpo no builder pg17; caller do build path é T3.3 (deferido honestamente com `#[allow(dead_code)]` + comentário). T3.1 (meta v3 codec) + T3.3 (build wiring) restam na Fase 3.
- **M59 (P1) — implement Fase 2: kernel Asymmetric-Hashing LUT16 SIMD (`vec::ah`)**: scorer per-candidate quase-grátis que ataca o gap ~25× QPS vs ScaNN (`theodb_rs/src/vec/ah.rs` + `vec/ah_tests.rs`, incluído via `include!`). Dado um query f32 + o codebook AQ (Fase 1), pré-computa **1 LUT por query** `LUT[m][16]` requantizada a int8 pela normalização afim **A/B do FAISS FastScan** (`código = round((val−B)/A)` global sobre os `m·16` parciais, maximizando A sem saturar o int8 nem estourar o acumulador i16); pontua um código por `Σ_i LUT[i][code_i]`. Caminho **batched pshufb** (`ah_score_block`): `_mm256_shuffle_epi8` faz 32 lookups paralelos/instrução, acumulação **int8→int16 widened** (`_mm256_cvtepi8_epi16`) contra overflow, layout transposto subquantizer-major (FAISS `bbs=32`); dispatch `is_x86_feature_detected!("avx2")` + fallback escalar (espelha o `simd_x86` do M58). Invariante de comprimento sempre-on (release) — sem OOB atravessando C (Rule 8). Decisão honesta medida: o pshufb **single-code** é mais lento que um table-lookup escalar (paga extract/subespaço), então `ah_score` single-code é escalar por design; o ganho é o **batched** — micro-bench mede **block32 4.75× mais rápido** que o loop escalar (1.6M scores, m=128). **7 pg_tests GREEN**: paridade pshufb↔escalar (m par/ímpar + tail, bit-idêntico), ADC ordena igual ao f32 (top-10 overlap + argmin), requant bounded (≤1 step) + degenerado sem panic, block32↔escalar, no-overflow (m=128 all-max), dim-mismatch rejeitado (typed Err), micro-bench. Wiring (branch no scan v3) nas Fases 3-4. Fases 3-5 (persistência v3, wiring, benchmark D3) restam.
- **M59 (P1) — implement Fase 1: codebook anisotrópico (`aq.rs`)**: quantizer PQ **anisotrópico** próprio, std-only (`theodb_rs/src/am/aq.rs`) — k-means por subespaço minimizando a score-aware anisotropic loss do ScaNN (Guo et al. 2020, `ℓ = h∥‖r∥‖² + h⊥‖r⊥‖²`; η=1 recupera isotrópico), 4-bit/16 centróides (LUT16 do kernel AH da Fase 2), `⌈m/2⌉` bytes/vetor, determinístico (seed), erros tipados (Rule 8), zero dep nova (reusa `crate::pq` Lloyd). Espelha `sbq.rs`. **11 pg_tests GREEN** (determinismo, η=1 isotrópico, η penaliza erro paralelo, ADC↔f32 correlaciona, roundtrip meta, + edge/negative). Wiring (caller scan/build) nas Fases 3-4 do plano. Fases 2-5 (kernel AH SIMD, persistência v3, wiring, benchmark) restam.
...
### Changed
- **M59 (P1) — discover: blueprint ganhou evidência web R0 (WebSearch/WebFetch)** (`.claude/knowledge-base/discoveries/blueprints/m59-anisotropic-ah-blueprint.md`): nova seção `## Evidência web (R0)` com **9 fontes abertas via WebFetch** e citadas com URL+autor/ano/venue ou símbolo de código — papers primários (Guo et al. ICML 2020 arXiv:1908.10396: **Theorem 3.2/3.3/3.4** da loss anisotrópica + `T=0.2⇒η≈4.125`; André et al. PVLDB 2015 fast-scan), OSS (FAISS FastScan Wiki: LUT16/`pshufb`/`bbs=32`/`A/B`; ScaNN blog: ~2× QPS vs 11 libs em glove-100; licenças ScaNN Apache-2.0 e FAISS MIT confirmadas da fonte) e blogs de engenharia (Milvus/Zilliz ScaNN: **5-6× QPS**, compressão 1/16, recall 0.71→0.94 c/ rerank; Medium Yamaguchi: "16 lookups/cycle"). **Correção de citação (R2):** os símbolos FAISS `pq4_fast_scan.*`/`simd_result_handlers.h` estavam com path stale — corrigidos para `faiss/impl/fast_scan/*` (resolvidos via `gh api`). Honesto (R3/R6): 2 PDFs primários ficaram `BLOCKED` no parser binário (números recuperados de espelho HTML ar5iv, nunca inventados); nenhum QPS atribuído ao nosso stack (permanecem `UNBENCHMARKED` até o gate D3).
- **Regra de rigor do discover: R0 — busca web obrigatória (REGRA MÁXIMA)** (`.claude/rules/discover-phd-rigor.md`): todo deep research (fase discover, `/deep-research`, varredura SOTA) DEVE usar WebSearch+WebFetch para buscar evidência em papers/OSS/blogs e citá-la — conhecimento interno + leitura de código local não bastam. Agentes de discover devem receber instrução explícita de buscar na web. Elevado a regra máxima pelo owner (2026-07-08).
...
### Fixed
- **M59 (P1) — codebook AQ v3 em múltiplas páginas (corrige `PageAddItem failed` a dim≥~170)** (`theodb_rs/src/am/hnsw_page.rs`, `build.rs`): `CREATE INDEX ... USING theodb_hnsw (... theodb_hnsw_cosine_ops) WITH (pq_subspaces=8, pq_bits=4, aq_threshold=4100)` num corpus **dim=768** falhava com `theodb am: PageAddItem failed (item too large / page full?)` — o codec meta v3 (T3.1) serializava o codebook AQ inteiro **inline na meta page** (item único do bloco 0), mas o codebook é `m·16·(dim/m)·4` bytes (**~48 KB** a dim=768), muito acima do BLCKSZ de 8 KB. Os testes unitários de T3.1 usaram dim pequeno (3, 8), onde o codebook cabe numa página, então não pegaram; a dim realista de embeddings (768/1536) estoura. **Fix:** o codebook AQ passa a ser persistido em **N páginas dedicadas** (uma por chunk de ≤ 8000 B), anexadas após as neighbor pages — exatamente como element/neighbor tuples já se espalham em múltiplas páginas via `Packed.pages`. A meta v3 carrega agora só um **descritor fixo de 13 bytes** `[aq_m:u8][cb_len:u32][cb_first:u32][cb_npages:u32]` que localiza essas páginas (o item da meta é sempre minúsculo, independe de `dim` → nunca estoura). `read_meta` (FFI) reconstrói o codebook lendo as N páginas e valida o comprimento contra `cb_len` (Rule 8: mismatch ⇒ typed Err "REINDEX v3 corruption", nunca silenciosamente errado). O fold do VACUUM re-empacota o codebook multi-página de forma posição-independente (relocável). **BACKWARD-COMPAT preservada**: v1 (f32-only, 45 bytes) e v2 (SBQ, codebook inline pequeno) inalterados byte-a-byte; nenhum teste SBQ/v1/v2 regride. Novo pg_test RED-then-GREEN `ambuild_dim768_pq_subspaces_multipage_codebook_no_page_overflow` (build v3 a dim=768 com codebook ≥2 páginas → sem `PageAddItem failed`, codebook round-trip bit-exato, η=4100 preservado, scan retorna o top-k exato).
...
### Added
- **M57 (P0) — harness a escala**: `run_m51_sbq_inline.py` agora carrega o corpus via COPY-streaming (reusa o loader do M55), viabilizando o benchmark SBQ vs f32 vs pgvector a **1M×768d** numa box dedicada (droplet DO 8-core/16GB) — o head-to-head decision-grade do gate de superioridade vetorial (M57). Geração numpy (`np.char.mod`) + `read()` que retorna um chunk inteiro (evita o gargalo `random.gauss` e o slicing O(n²) do buffer a 1M).
- **M56 DoD-1 fase 2 (RepairGraph / in-place insert) — fundação**: `hnsw_page::find_reusable_slot` acha um slot de element tombstonado reusável para um novo nó (entrada do slot-reuse no `aminsert`, padrão `pgvector/hnswinsert.c`); `page::read_all_page_items_with_off` expõe o `OffsetNumber` de cada item. **T1.2:** `page::modify_item_at` (edita UM item por `(blk,off)` sob GenericXLog, crash-safe) + `hnsw_page::write_reused_element` (revive o slot tombstonado com novo tid+vetor, `deleted=0`, `version++`, mantendo level+nbr-addr — Z assume a posição de grafo de X; v1/f32 por ora). **T1.3:** `hnsw_page::set_ground_neighbors_inplace` escreve os slots de vizinhos ground de um nó in-place (byte-edit sob GenericXLog, round-trip por `decode_neighbors`). **T2.1:** `hnsw_page::insert_search_ground` — a busca on-disk do insert (descida greedy nas camadas superiores + ground search com ef_construction, reusa a maquinaria do `traverse`), retorna os `m0` addrs de elementos vivos mais próximos que Z vai linkar. **T3 (end-to-end):** `hnsw_page::insert_inplace` orquestra o insert in-place com slot-reuse (find slot → search → revive → set ground neighbors → backward-link nos vizinhos com vaga); `aminsert` tenta o reuso ANTES de crescer o pending (padrão hnswinsert.c). Claim atômico por slot (`write_reused_element` só revive `deleted=1` sob buffer-EXCLUSIVE) torna inserts concorrentes seguros. **DoD-1 (reúso de slot no aminsert) entregue e testado end-to-end** (aceitação: tombstones reusados count→0 + novas linhas linkadas e achadas pelo índice). v1/f32; caveat honesto: backward-link pode ter lost-update sob inserts concorrentes ao mesmo vizinho (assimétrico, cura no fold); v2/SBQ e níveis altos = follow-up. **T4.2 crash-e2e (`scripts/m56-slot-reuse-crash-e2e.sh`):** build → DELETE+VACUUM (tombstone) → INSERT (reusa slots) → SIGKILL → restart → as novas linhas sobrevivem e continuam linkadas, as deletadas ficam fora — o `insert_inplace` é WAL-durável (GenericXLog). Design completo do subsistema (search+link on-disk, crash-safe, ADRs de level-matching e ordenação) em `knowledge-base/plans/m56-repairgraph-inplace-insert-plan.md`. Ainda NÃO wired no `aminsert` (restam T1.2/T1.3/T2.1/T3/T4).
...
- **M56 DoD-4 — teto de memória do BUILD**: `HnswIndex::build_owned` **consome** o corpus (move os `Vec<f32>`, sem clonar), então `CREATE INDEX`/`REINDEX` de 1M×768d mantém os vetores f32 residentes **uma vez** (pico ~1×O(N)) em vez de duas (corpus + cópia = ~2×O(N)). `ambuild_hnsw` e o fold do VACUUM passam o resultado do heap-scan direto para `build_owned`; o wrapper `build_cancellable(&[…])` (testes/bench/ANN ad-hoc) continua clonando — escala minúscula. O grafo HNSW é intrinsecamente O(N) em vetores; o DoD-4 remove a **segunda cópia evitável**, não o piso intrínseco.
...
### Changed
- **M56 fase 2 — slot-reuse é OFF por default (achado do benchmark, Regra 3):** o benchmark de churn (`docs/benchmarks/m56-slot-reuse-churn.md`) mediu que o slot-reuse **suprime o gatilho de compactação** (consome os tombstones antes do threshold → o fold que REPARA o grafo nunca dispara) e o **recall@10 despenca (~0.57 vs ~0.95** do caminho default navigate-through+fold). Slot-reuse limita o TAMANHO do índice (0% vs 284% de crescimento) mas troca isso por RECALL — troca ruim. Por isso `theodb.hnsw_slot_reuse` **default OFF** (opt-in); o navigate-through + fold-compaction (DoD 2/3, recall estável) permanece o default correto. **Fix aplicado** (a) reusar só slots de nível-0 não-entry (`find_reusable_slot` exact-level + skip-entry) + (b) gatilho de compactação por CHURN (`count_churned`=`version>0`, tombstones+reusados) para o fold reparar sob reuso → recall corrigido (0.57→**0.955**). Porém o benchmark então mostrou benefício líquido MARGINAL (size 1.04–1.18×, recall nunca melhor que o navigate-through+fold) → default OFF mantido por evidência.
- **Layout do element tuple do `theodb_hnsw`**: os 2 bytes de pad (offsets 2–3, sempre zero até M55) passam a carregar `deleted` (flag de tombstone) e `version` (hook forward-compat p/ slot-reuse da fase 2). Índices v1/v2 pré-M56 permanecem compatíveis (pad=0 decodifica como vivo/versão 0); nenhum REINDEX é exigido. Nova GUC `theodb.hnsw_tombstone_compact_pct` (0–100, default 20) controla o gatilho de compactação (M56). Nova GUC `theodb.hnsw_slot_reuse` (bool, default on) liga/desliga o reuso de slot in-place no `aminsert` (kill-switch + toggle A/B do benchmark de churn, M56 fase 2).
...
- SBQ codebook serializa/desserializa para meta bytes (`SbqQuantizer::to_meta_bytes`/`from_meta_bytes`, `sbq.rs`) — base do layout v2 SBQ do M51 (T1.1); validação de tamanho fail-fast (Rule 8). (M51)
